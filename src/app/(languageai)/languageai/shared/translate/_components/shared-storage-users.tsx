...
const SharedTranslateStorageUsers = ({isFetching, searchedEmail, sharedUsers}: SharedTranslateStorageUsersProps) => {
    ...
    if (sharedUsers.length === 0 && searchedEmail) {
        return <Button className="w-full shadow text-pretty" variant="secondary">
            <LuPlus />
    ...
    if (sharedUsers.length === 0) {
        return <div className={cn(buttonVariants({variant: "ghost"}), "w-full")}>
   ...
   return (
       <div className="max-h-96 overflow-y-auto">
           ...
           <div className={cn(buttonVariants({variant: "ghost"}), "w-full justify-between")} key={user.id}>
              <span>{user.shared_user_email} ({user.permission})</span>
           </div>
           )}
       </div>
   )
