import {
    findLanguageaiSubscriptionPendingPayment
} from "@/lib/api/languageai-subscriptions/find-languageai-subscription-pending-payment";
    ...
    try {
        ...
        return (
            <div className="border p-4 bg-destructive/25 shadow rounded-lg max-w-xs">
                ...
                <div className="mb-2"> Pay before: {formatDateToIndonesian(pendingPayment.data.expired_at, true)}</div>
                {pendingPayment.data.doku_response &&
                    <Link href={pendingPayment.data.doku_response.response.payment.url} className={cn(buttonVariants(), "w-full")}>
                          Payment Link
                    </Link>}
            </div>
        )
