    ...
    <ul className="flex flex-col gap-2">
      <li
        className={cn(
          ...
          plan.category === "text" && "font-bold",
        )}
      >
        <LuLanguages />
        <span>
          {plan.translation_limit
            ? `${plan.translation_limit} translation per month`
      ...
      >
        ...
        <span>
          {plan.checkbot_limit
            ? `${plan.translation_limit} checkbot per month`
            : "Unlimited checkbot"}
        </span>
      </li>
      <li
        className={cn(
          "flex items-center gap-4",
          plan.category === "audio" && "font-bold",
        )}
      >
        <LuAudioLines />
        <span>
          {plan.text_to_speech_limit
            ? `${plan.text_to_speech_limit} text to speech per month`
      ...
      >
        ...
        <span>
          {plan.speech_to_text_limit
            ? `${plan.speech_to_text_limit} speech to text minutes/month`
      ...
      <li className="flex items-center gap-4">
        ...
        <span>
          {plan.storage_limit
            ? `${plan.storage_limit} storage rows`
            : "Unlimited storage record"}
        </span>
      </li>
      <li className="flex items-center gap-4">
        <LuClock />
        <span>
          {plan.history_limit
            ? `${plan.history_limit} latest history`
            : "Unlimited history record"}
        </span>
      </li>
    </ul>
  );
