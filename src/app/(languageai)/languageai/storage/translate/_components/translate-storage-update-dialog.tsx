...
const TranslateStorageUpdateDialog = ({ row }: TTranslateStorageUpdateDialogProps) => {
       ...
       <Dialog>
           ...
           <DialogContent className="lg:max-w-5xl">
               ...
               <DialogTitle className="font-semibold">Update Translation</DialogTitle>
                   <DialogClose className={buttonVariants({variant: "outline", size: "icon"})}><LuX /> </DialogClose>
               </div>
               <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 mb-4">
                   <div>
                       <div className="text-sm mb-2">{row.original.content_language || "Detect Language"}</div>
                       <div className="h-[30vh] lg:h-[40vh] text-sm">{row.original.content}</div>
                   </div>
                   <div>
                       <div className="text-sm mb-2 capitalize">{row.original.target_language}</div>
                       <Textarea autoFocus className="h-[30vh] lg:h-[40vh] resize-none" value={value} onChange={(e)=> setValue(e.target.value)} />
                   </div>
               </div>
               <div className="flex items-center justify-end">
                   <DialogClose className={buttonVariants()} onClick={onSaveClick}><LuSave/> Save </DialogClose>
               </div>
           </DialogContent>
       </Dialog>
    )
