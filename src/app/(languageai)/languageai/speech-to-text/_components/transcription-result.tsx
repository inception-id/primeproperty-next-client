...
import {createSpeechToTextStorage} from "@/lib/api/speech-to-text/createTextToSpeechStorage";

const TranscriptionResult = () => {
    const {text, updateStore, speechToTextId, isLoading} = useTranscriptionStore(
        useShallow((state) => ({
    ...
    const onSaveClick = async () => {
       try {
          const savedTranscript = await createSpeechToTextStorage(speechToTextId, text);
