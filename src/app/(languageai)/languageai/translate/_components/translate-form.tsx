...
const TranslateForm = () => {
    const {complete, isLoading} = useContext<UseCompletionHelpers>(TranslateContext)
    const handleAction =async (formData: FormData) => {
        ...
        try {
        await complete(translateContent, {body: {system: createTranslateSystemPrompt(contentLanguage, targetLanguage)}})
        } catch (e) {
           console.error(e);
    ...
    return (
        <form action={handleAction} className="flex flex-col">
            <Textarea name="translate_content" placeholder="Enter text" className="focus-visible:ring-0 focus-visible:ring-offset-0 h-96 lg:h-[90vh] lg:flex-1 resize-none" />
