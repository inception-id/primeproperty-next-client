...
const TranslateSharingInfoDialog = ({
  showDialog,
}: TranslateSharingInfoDialogProps) => {
  return (
    <Dialog open={showDialog}>
      <DialogContent
        onEscapeKeyDown={handleTranslateInfoClose}
        onOverlayClick={handleTranslateInfoClose}
      >
        ...
        <DialogDescription className="flex items-center gap-2 mb-4">
          You can now share your translation result by clicking share icon and
          invite your friends
        </DialogDescription>
        <div className="flex items-center justify-end">
          <DialogClose
            className={cn(buttonVariants())}
            onClick={async () => await handleTranslateInfoClose()}
          >
            Close
          </DialogClose>
        </div>
