...
export const createSupertokensSession = async (supertokensUserId: string, userData: TUser): Promise<TResponse> => {
    try {
        const payload = {
        ...
        return await fetchSupertokens("/recipe/session", {method: "POST", body: JSON.stringify(payload)});
    } catch (e) {
        throw e
    }
}